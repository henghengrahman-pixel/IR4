...
export function isAllowedPredictionLeague(input = {}) {
  ...
  const countryOnly = normText(country);

  if (!leagueOnly) return false;

  if (
    BLOCKED_KEYWORDS.some(word =>
      combined.includes(normText(word))
    )
  ) {
    return false;
  }
  ...
  const allow =
    envList("PREDICTION_ALLOWED_LEAGUES").length
      ? envList("PREDICTION_ALLOWED_LEAGUES")
      : DEFAULT_ALLOWED_LEAGUES;

  const matchedLeague = allow.some(label => {
  ...
  if (
    ...
    !PRIORITY_COUNTRIES.some(c =>
      countryOnly.includes(normText(c))
    )
  ) {
    if (
      !combined.includes("champions league") &&
      !combined.includes("europa")
    ) {
      return false;
    }
  }
...
export function formatKickoffWib(iso) {
  ...
  try {
    ...
    const dm = new Intl.DateTimeFormat("id-ID", {
      timeZone: "Asia/Jakarta",
      day: "2-digit",
      month: "2-digit"
    }).format(d);
...
function estimateBySeed(raw) {
  ...
  const homePower =
    45 + (hashNum(`${home}-home`) % 55);

  const awayPower =
    45 + (hashNum(`${away}-away`) % 55);

  const diff =
    (homePower + 8 + (seed % 7)) - awayPower;
  ...
  const confidence = Math.max(
    ...
    Math.min(
      88,
      Math.round(58 + Math.abs(diff) * 1.15)
    )
  );
