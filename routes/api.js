...
import { isAllowedLeague, getLeaguePriority, normalizePrediction } from "../helpers/prediction-normalizer.js";
...
async function getFixturesAll(date) {
  ...
  const out = resp.map(m => ({ league: m.league, fixture: m.fixture, teams: m.teams, goals: m.goals, score:m.score }));
...
router.get("/api/fixtures", async (req, res) => {
  try {
    ...
    const list = fixtures.filter(fx => isAllowedLeague(fx));
    ...
    for (const [leagueId, info] of byLeague) {
      ...
      let rows = (info.rows || []).map(m => {
        const n = normalizePrediction(m);
        const h = m.teams?.home || {}, a = m.teams?.away || {};
        return {
          fixtureId: m.fixture?.id,
          homeId: h.id,
          awayId: a.id,
          kickoff: n.kickoffWib,
          match: `${n.homeName} vs ${n.awayName}`,
          score: n.score,
          tip: n.prediction === "1" ? n.homeName : n.prediction === "2" ? n.awayName : "Draw",
          pick: n.prediction,
          ou: n.overUnder,
          odds: n.odds,
          ...
          normalized: n,
          _prio: matchPriorityFromLabel(`${n.homeName} vs ${n.awayName}`)
        };
      }).filter(r => r.homeLogo && r.awayLogo);
    ...
    groups.sort((a, b) => {
      const wa = getLeaguePriority(a.rawTitle || a.displayTitle);
      const wb = getLeaguePriority(b.rawTitle || b.displayTitle);
