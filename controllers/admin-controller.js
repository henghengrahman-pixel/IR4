...
import { normalizePrediction, predictionToLegacyRow } from "../helpers/prediction-normalizer.js";
...
function parsePredictions(raw = ""){
  ...
  try {
    ...
    return parsed.map(league => ({
      league: String(league.league || league.leagueName || "Liga").trim(),
      matches: Array.isArray(league.matches) ? league.matches.map(match => {
        const n = match?.normalized || normalizePrediction({ ...match, leagueName: league.league || league.leagueName });
        return { ...predictionToLegacyRow(n), ...match, normalized:n };
      }).filter(m => m.match) : []
...
function assetFromRequest(req, field, fallback = ""){
  const file = req.files?.[field]?.[0] || (field === "thumbnailFile" ? req.file : null);
  if (file?.filename) return `/uploads/${file.filename}`;
  const urlField = field.replace("File", "Url");
  return req.body?.[field]?.trim?.() || req.body?.[urlField]?.trim?.() || fallback || "";
}
...
async function removeUpload(publicUrl=""){
  if (!String(publicUrl).startsWith("/uploads/")) return;
  try { await fs.unlink(path.join(uploadDir, path.basename(publicUrl))); } catch {}
}
...
export async function slidesCreate(req,res){ const slides = await getSlides(); slides.push({ id:id('slide'), title:req.body.title?.trim()||'', subtitle:req.body.subtitle?.trim()||'', buttonText:req.body.buttonText?.trim()||'', image:assetFromRequest(req,'imageFile') || req.body.image?.trim() || '', link:req.body.link?.trim()||'', order:Number(req.body.order||slides.length+1), active:toBool(req.body.active) }); await saveSlides(slides); res.redirect('/admin/slides'); }
export async function slidesUpdate(req,res){ const slides = await getSlides(); await saveSlides(slides.map(item => item.id === req.params.id ? { ...item, title:req.body.title?.trim()||'', subtitle:req.body.subtitle?.trim()||'', buttonText:req.body.buttonText?.trim()||'', image:assetFromRequest(req,'imageFile', item.image) || req.body.image?.trim() || item.image || '', link:req.body.link?.trim()||'', order:Number(req.body.order||item.order||0), active:toBool(req.body.active) } : item)); res.redirect('/admin/slides'); }
export async function slidesDelete(req,res){ const slides = await getSlides(); const old = slides.find(item=>item.id === req.params.id); await removeUpload(old?.image); await saveSlides(slides.filter(item=>item.id !== req.params.id)); res.redirect('/admin/slides'); }
...
export async function settingsUpdate(req,res){
  ...
  await saveSettings({
    siteName:req.body.siteName?.trim() || "", siteSubtitle:req.body.siteSubtitle?.trim() || "",
    logoUrl:assetFromRequest(req,'logoFile', current.logoUrl), footerLogoUrl:assetFromRequest(req,'footerLogoFile', current.footerLogoUrl), faviconUrl:assetFromRequest(req,'faviconFile', current.faviconUrl),
    backgroundDesktop:assetFromRequest(req,'backgroundDesktopFile', current.backgroundDesktop), backgroundMobile:assetFromRequest(req,'backgroundMobileFile', current.backgroundMobile), heroBanner:assetFromRequest(req,'heroBannerFile', current.heroBanner),
    telegram:req.body.telegram?.trim() || "", whatsapp:req.body.whatsapp?.trim() || "", livechat:req.body.livechat?.trim() || "",
    loginUrl:req.body.loginUrl?.trim() || "", registerUrl:req.body.registerUrl?.trim() || "", runningText:req.body.runningText?.trim() || "", footerText:req.body.footerText?.trim() || "",
    seoTitle:req.body.seoTitle?.trim() || "", seoDescription:req.body.seoDescription?.trim() || "", seoKeywords:req.body.seoKeywords?.trim() || "",
    metaTitle:req.body.seoTitle?.trim() || "", metaDescription:req.body.seoDescription?.trim() || "",
    socials:{ facebook:req.body.facebook?.trim()||"", instagram:req.body.instagram?.trim()||"", twitter:req.body.twitter?.trim()||"", tiktok:req.body.tiktok?.trim()||"", youtube:req.body.youtube?.trim()||"" }
  });
