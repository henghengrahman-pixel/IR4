...
export async function settingsUpdate(req,res){ await saveSettings({ siteName:req.body.siteName?.trim(), loginUrl:req.body.loginUrl?.trim(), registerUrl:req.body.registerUrl?.trim(), runningText:req.body.runningText?.trim(), logoUrl:req.body.logoUrl?.trim(), footerText:req.body.footerText?.trim(), metaTitle:req.body.metaTitle?.trim(), metaDescription:req.body.metaDescription?.trim(), backgroundImage:req.body.backgroundImage?.trim() }); res.redirect('/admin/settings'); }
